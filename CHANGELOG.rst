...
* Removed NLX inway configuration integration (#949)
...
.. warning::
   ...
   **This has an impact on existing installations**
   ...
   **We strongly advise to test this upgrade on a test/staging environment before rolling
   ...
   .. tabs::
     ...
     .. group-tab:: single-server, Docker

       If you are deploying with the ansible playbooks, then upgrading to the latest
       version of the collection should be sufficient. An Ansible task is included that
       will configure the file system permissions recursively for the affected volumes.

     .. group-tab:: single-server, Podman

        TODO - unclear what needs to be done since we don't have access to a podman
        testing environment.

     .. group-tab:: Kubernetes, Ansible
