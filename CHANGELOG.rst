...
.. note::
  ...
  The default for several environment variables for configuring  for ``django-log-outgoing-requests``
  have changed (see :ref:`installation_env_config` > Logging)
...
* [:open-zaak:`2308`] Ensure ``zaak-gemuteerd`` cloud event is emitted for all changes
  ...
  ``Zaak.laatstGemuteerd`` is updated for the same changes.
...
* Update notification retry parameter defaults (see :ref:`delivery_guarantees`)
