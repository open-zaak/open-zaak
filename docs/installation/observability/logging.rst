...
Celery tasks
^^^^^^^^^^^^
* ``empty_applications_deleted``: Indicates that Applicaties that had no authorizations and also no superuser rights were deleted automatically (INFO). Additional context: ``apps_to_delete``.
