.. _development_howtos_aws_s3_storage:

Using AWS S3 storage for Documenten API
=======================================

In order to test AWS S3 storage for the Documenten API locally, make sure the environment
variable ``DOCUMENTEN_API_BACKEND`` is set to ``aws_s3_storage``.
...
If you want to connect with a real AWS S3 storage instance, make sure to follow the instructions
in the :ref:`installation docs <installation_documenten_aws_s3_storage>` to configure
an app and service principal in AWS and then set the correct values as described for the
envvars.
...
.. note::
    ...
    When making requests to the AWS S3 storage API, the filename is included in the
