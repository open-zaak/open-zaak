...
class BesluitInformatieObjectSyncMixin:
    ...
    def setUp(self):
        ...
        patcher_sync_create = patch('openzaak.brc.sync.signals.sync_create_bio')
        self.mocked_sync_create_bio = patcher_sync_create.start()
        self.addCleanup(patcher_sync_create.stop)

        patcher_sync_delete = patch('openzaak.brc.sync.signals.sync_delete_bio')
...
class BesluitSyncMixin:
    ...
    def setUp(self):
        ...
        patcher_sync_create = patch('openzaak.brc.sync.signals.sync_create_besluit')
        self.mocked_sync_create_besluit = patcher_sync_create.start()
        self.addCleanup(patcher_sync_create.stop)

        patcher_sync_delete = patch('openzaak.brc.sync.signals.sync_delete_besluit')
