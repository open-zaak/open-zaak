...
class GebruiksrechtenViewSet(
    ...
    def partial_update(self, request, *args, **kwargs):
        response = super().partial_update(request, *args, **kwargs)
        logger.info(
            "gebruiksrechten_partial_updated",
            client_id=request.jwt_auth.client_id,
            uuid=kwargs.get("uuid"),
        )
        return response
...
class ReservedDocumentViewSet(viewsets.ViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        output_data = self.perform_create(bronorganisatie, identificaties, aantal)

        return Response(output_data, status=status.HTTP_201_CREATED)

    def perform_create(self, bronorganisatie, identificaties, aantal):
        if aantal == 1:
