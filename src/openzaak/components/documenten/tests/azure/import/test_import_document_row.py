...
class ImportDocumentRowTests(
    AzureBlobStorageMixin, ImportTestMixin, VCRMixin, TestCase
):
    ...
    def test_minimum_fields(self):
        ...
        # assert that it's stored in azure
        self.assertTrue(documenten_storage.exists(str(imported_path)))
        # Make sure the AZURE_LOCATION is not prepended twice
    ...
    def test_import_cannot_use_arbitrary_filepaths(self):
        ...
        for filepath in rejected_filepaths:
            with self.subTest(filepath=filepath):
                ...
                # assert that it's not stored in azure
                self.assertFalse(documenten_storage.exists(str(imported_path)))
                # Make sure the AZURE_LOCATION is not prepended twice
                self.assertFalse(str(imported_path).startswith("documenten/documenten"))
                # assert the file does not exist on disk
...
class ImportDocumentRowWithoutOverwriteTests(
    ...
    def test_file_already_exists_in_storage(self, _):
        ...
        # Make sure the file already exists in Azure storage
        ...
        self.assertTrue(actual_file_path.startswith("uploads/2030/01/already_exists"))
