...
from .mixins import AWSS3torageMixin, upload_to


@patch("privates.fields.PrivateMediaFileField.generate_filename", upload_to)
@tag("gh-2282", "aws-s3-storage")
@temp_private_root()
class SmallFileUpload(JWTAuthMixin, VCRMixin, AWSS3torageMixin, APITestCase):
    ...
    def test_create_eio(self):
        ...
        * bestandsdelen objects are not created
        ...
        with patch(
            ...
            return_value=UUID("ef0f8f1b-f381-463e-9f01-ac33ea135925"),
        ):
    ...
    def test_create_empty_file(self):
        ...
        * bestandsdelen objects are not created
        ...
        with patch(
            "openzaak.components.documenten.models._uuid.uuid4",
            return_value=UUID("6c6cfc37-53f1-4804-9f3d-0700a9cc1c54"),
        ):
            response = self.client.post(list_url, content)
    ...
    def test_update_eio_file(self):
        ...
        * bestandsdelen objects are not created
        ...
        eio = EnkelvoudigInformatieObjectFactory.create(
            informatieobjecttype=self.informatieobjecttype,
            inhoud__filename="filename2.bin",
            uuid="5db29fcb-1585-44f9-a16e-766789795029",
        )
        detail_url = reverse(eio)
        lock_url = get_operation_url("enkelvoudiginformatieobject_lock", uuid=eio.uuid)
        ...
        with patch(
            "openzaak.components.documenten.models._uuid.uuid4",
            return_value=UUID("5db29fcb-1585-44f9-a16e-766789795029"),
        ):
            # update metadata
            response = self.client.patch(
                detail_url,
    ...
    def test_update_eio_file_set_empty(self):
        ...
        * bestandsdelen objects are not created
        ...
        eio = EnkelvoudigInformatieObjectFactory.create(
            informatieobjecttype=self.informatieobjecttype,
            inhoud__filename="test_update_eio_file_set_empty.bin",
        )
        detail_url = reverse(eio)
        lock_url = get_operation_url("enkelvoudiginformatieobject_lock", uuid=eio.uuid)
    ...
    def test_update_eio_only_size(self):
        ...
        * 400 status because the new bestandsomvang is not related to the file size
        ...
        eio = EnkelvoudigInformatieObjectFactory.create(
            informatieobjecttype=self.informatieobjecttype,
            inhoud__filename="test_update_eio_only_size.bin",
        )
        detail_url = reverse(eio)
        lock_url = get_operation_url("enkelvoudiginformatieobject_lock", uuid=eio.uuid)
    ...
    def test_update_eio_only_file_without_size(self):
        ...
        * 400 status because the new file size is not related to the bestandsomvang
        ...
        eio = EnkelvoudigInformatieObjectFactory.create(
            informatieobjecttype=self.informatieobjecttype,
            inhoud__filename="test_update_eio_only_file_without_size.bin",
        )
        detail_url = reverse(eio)
        lock_url = get_operation_url("enkelvoudiginformatieobject_lock", uuid=eio.uuid)
    ...
    def test_update_eio_put(self):
        ...
        * bestandsdelen objects are not created
        ...
        eio = EnkelvoudigInformatieObjectFactory.create(
            informatieobjecttype=self.informatieobjecttype,
            uuid="73aefdb3-ce05-4dc2-a9d3-8e9833618613",
            inhoud__filename="test_update_eio_put.bin",
        )
        detail_url = reverse(eio)
        lock_url = get_operation_url("enkelvoudiginformatieobject_lock", uuid=eio.uuid)
        ...
        with patch(
            "openzaak.components.documenten.models._uuid.uuid4",
            return_value=UUID("73aefdb3-ce05-4dc2-a9d3-8e9833618613"),
        ):
...
class LargeFileAPITests(JWTAuthMixin, AWSS3torageMixin, VCRMixin, APITestCase):
    component = ComponentTypes.drc
    scopes = [
        SCOPE_DOCUMENTEN_LOCK,
        SCOPE_DOCUMENTEN_AANMAKEN,
    ...
    def test_create_eio_full_process(self):
        ...
        * file is downloadable via the file link
        ...
        with patch(
            "openzaak.components.documenten.models._uuid.uuid4",
            return_value=UUID("c8f138c5-c0d2-4960-8f51-f5a399a716a2"),
        ):
