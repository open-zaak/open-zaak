...
class ImportDocumentTestCase(S3torageMixin, ImportTestMixin, VCRMixin, TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.informatieobjecttype = InformatieObjectTypeFactory.create(
            concept=False, uuid="06826241-f940-469f-bc17-702a8be0d120"
        )
    ...
    def test_simple_import(self):
        ...
        with patch.object(field, "upload_to", new="uploads/test/"):
    ...
    def test_database_connection_loss(self, mocked_bulk_create, mocked_uuid):
        ...
        field = EnkelvoudigInformatieObject.inhoud.field
        with patch.object(field, "upload_to", new="uploads/test/"):
            default_path = get_default_path(field)

        for filename in absent_files:
            expected_path = Path(default_path / filename)

            with self.subTest(filename=filename):
                self.assertFalse(documenten_storage.exists(expected_path))
    ...
    def test_integrity_error(self, mocked_bulk_create, mocked_uuid):
        ...
        random_eios = [
            EnkelvoudigInformatieObjectFactory.create(
                informatieobjecttype=self.informatieobjecttype,
                inhoud__filename=f"test_integrity_error{i}.bin",
            )
            for i in range(2)
        ]
