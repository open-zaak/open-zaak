...
class EnkelvoudigInformatieObjectAdminTests(VCRMixin, S3torageMixin, WebTest):
    ...
    def test_create_informatieobject_save_s3_request_exception(self):
        """
        If no connection could be made with S3, the admin form should raise errors
        ...
        form["taal"] = "nld"
        form["inhoud"] = Upload(
            "test_create_informatieobject_save_s3_timeout.txt", b"foo"
        )
        ...
        self.assertEqual(
            ...
            [
                [
                    _(
                        "Something went wrong while trying to write the file to S3 Storage"
                    )
                ],
            ],
        )
