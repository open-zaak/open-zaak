...
def populate_zaak_vertrouwelijkheidaanduiding_integer_field(apps, schema_editor):
    Zaak = apps.get_model("zaken", "Zaak")
    BATCH_SIZE = 2000
    processed = 0
    total = Zaak.objects.count()
    batch = []
    for instance in Zaak.objects.iterator():
        ...
        if len(batch) >= BATCH_SIZE:
            Zaak.objects.bulk_update(batch, ["_vertrouwelijkheidaanduiding"])
            batch.clear()
...
def populate_zaak_vertrouwelijkheidaanduiding_char_field(apps, schema_editor):
    Zaak = apps.get_model("zaken", "Zaak")
    BATCH_SIZE = 2000
    processed = 0
    queryset = Zaak.objects.filter(vertrouwelijkheidaanduiding="")
    ...
    for instance in queryset.iterator():
        ...
        if len(batch) >= BATCH_SIZE:
            Zaak.objects.bulk_update(batch, ["vertrouwelijkheidaanduiding"])
            batch.clear()
            processed += BATCH_SIZE
            logger.info(
                "Processed %s / %s EnkelvoudigInformatieObjecten", processed, total
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(
            populate_zaak_vertrouwelijkheidaanduiding_integer_field,
            populate_zaak_vertrouwelijkheidaanduiding_char_field,
        ),
    ]
