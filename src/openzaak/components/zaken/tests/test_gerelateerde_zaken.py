...
class LocalGerelateerdeZakenTests(JWTAuthMixin, APITestCase):
    ...
    def test_create_local_gerelateerde_zaak(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "url": zaak_url,
                    # `aardRelatie` and `overigeRelatie` are injected by the serializer
                    # to ensure backward compatibility with `relevanteAndereZaken`
                    "aardRelatie": "overig",
    ...
    def test_create_local_gerelateerde_zaak_and_relevante_andere_zaken(self):
        """
        Verify that when supplying both `relevanteAndereZaken` and `gerelateerdeZaken`,
        `relevanteAndereZaken` takes precedence over `gerelateerdeZaken`
        """
        zaaktype = ZaakTypeFactory.create(concept=False)
        zaaktype_url = f"http://testserver.com{reverse(zaaktype)}"
        zaak = ZaakFactory.create(zaaktype=zaaktype)
        ...
        zaak_url2 = f"http://testserver.com{reverse(zaak2)}"

        with self.subTest("both defined as non-empty lists"):
            response = self.client.post(
                self.list_url,
                {
                    "zaaktype": zaaktype_url,
        ...
        with self.subTest("relevanteAndereZaken defined as empty list"):
            response = self.client.post(
                self.list_url,
                {
                    "zaaktype": zaaktype_url,
            ...
            self.assertEqual(response.json()["relevanteAndereZaken"], [])
            self.assertEqual(response.json()["gerelateerdeZaken"], [])

    def test_update_local_gerelateerde_zaak_and_relevante_andere_zaken(self):
        """
        Verify that when supplying both `relevanteAndereZaken` and `gerelateerdeZaken`,
        `relevanteAndereZaken` takes precedence over `gerelateerdeZaken`
        """
        existing_zaak = ZaakFactory.create()
        zaak = ZaakFactory.create()
        zaak_url = f"http://testserver.com{reverse(zaak)}"
