...
class BesluitVerwerkenAuthTests(JWTAuthMixin, APITestCase):
    url = reverse_lazy("zaken:verwerkbesluit-list")
    ...
    def _add_besluiten_auth(self, besluittype=None, zaaktype=None, use_default=True):
...
class BesluitVerwerkenValidationTests(JWTAuthMixin, APITestCase):
    ...
    def test_verwerk_besluit(self):
        ...
        expected_besluit_url = reverse(besluit, namespace="zaken")
        expected_besluitinformatieobject_url = reverse(
            besluitinformatieobject, namespace="zaken"
        )
