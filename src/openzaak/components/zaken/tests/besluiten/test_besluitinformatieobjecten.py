...
class BesluitInformatieObjectAPITests(JWTAuthMixin, APITestCase):
    list_url = reverse_lazy(
        "zaken:besluitinformatieobject-list", kwargs={"version": "1"}
    )
    ...
    def test_create(self):
        ...
        expected_url = reverse(stored_object, namespace="zaken")
    ...
    def test_filter_by_besluit(self):
        bio = BesluitInformatieObjectFactory.create()
        besluit_url = reverse(bio.besluit, namespace="zaken")
        bio_list_url = reverse("zaken:besluitinformatieobject-list")
