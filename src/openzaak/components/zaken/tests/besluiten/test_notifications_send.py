...
# and change the NAMESPACE & BESLUITTYPE_NAMESPACE to zaken.
...
class SendNotifTestCase(NotificationsConfigMixin, JWTAuthMixin, APITestCase):
    ...
    @override_settings(SEND_NOTIFICATIONS_ON_DEPRECATED_CHANNELS=False)
    def test_send_notif_create_besluit_with_zaak_do_not_send_on_deprecated_channels(
        self, mock_notif
    ):
        zaak = ZaakFactory.create()
        zaak_url = reverse(zaak)
    ...
    @override_settings(SEND_NOTIFICATIONS_ON_DEPRECATED_CHANNELS=False)
    def test_send_notif_create_besluit_without_zaak_do_not_send_on_deprecated_channels(
        self, mock_notif
    ):
        besluittype = BesluitTypeFactory.create(concept=False)
        besluittype_url = reverse(besluittype, namespace=self.BESLUITTYPE_NAMESPACE)
                ...
                call(
                    {
                        ...
                        "kenmerken": {
                            ...
                            "zaaktype": f"http://testserver{reverse(besluit.zaak.zaaktype, namespace='zaken')}",
                            "zaaktype.catalogus": f"http://testserver{reverse(besluit.zaak.zaaktype.catalogus, namespace='zaken')}",
    ...
    @override_settings(SEND_NOTIFICATIONS_ON_DEPRECATED_CHANNELS=False)
    def test_send_notif_verwerk_besluit_with_zaak_do_not_send_on_deprecated_channels(
        self, mock_notif
    ):
        zaak = ZaakFactory.create()
        zaak_url = reverse(zaak)
    ...
    @override_settings(SEND_NOTIFICATIONS_ON_DEPRECATED_CHANNELS=False)
    def test_send_notif_create_besluitinformatieobject_with_zaak_do_not_send_on_deprecated_channels(
        self, mock_notif
    ):
        zaak = ZaakFactory.create()
        zaak_url = reverse(zaak)
