...
from ..factories import ZaakFactory, ZaakEigenschapFactory
...
class ZaakEigenschapAdminTests(AdminTestMixin, TestCase):
    ...
    def test_create_zaakeigenschap(self):
        ...
        zaakeigenschap_url = get_operation_url("zaakeigenschap_read", uuid=zaakeigenschap.uuid, zaak_uuid=zaak.uuid)
    ...
    def test_change_zaakeigenschap(self):
        ...
        zaakeigenschap_url = get_operation_url(
            "zaakeigenschap_read", uuid=zaakeigenschap.uuid, zaak_uuid=zaakeigenschap.zaak.uuid
        )
        zaak_url = get_operation_url("zaak_read", uuid=zaakeigenschap.zaak.uuid)
        change_url = reverse('admin:zaken_zaakeigenschap_change', args=(zaakeigenschap.pk,))
        data = {
    ...
    def test_delete_zaakeigenschap_action(self):
        ...
        data = {'action': 'delete_selected', '_selected_action': [zaakeigenschap.id], 'post': 'yes'}
    ...
    def test_delete_zaakeigenschap(self):
        ...
        zaak_url = get_operation_url("zaak_read", uuid=zaakeigenschap.zaak.uuid)
        delete_url = reverse('admin:zaken_zaakeigenschap_delete', args=(zaakeigenschap.pk,))
        data = {'post': 'yes'}
