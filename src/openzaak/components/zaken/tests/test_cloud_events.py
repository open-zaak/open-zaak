...
class IncomingZaakCloudEventTests(JWTAuthMixin, APITestCase):
    ...
    def test_incoming_example_event(self):
        event = CloudEvent(
            {"type": ZAAK_GELINKT, "source": "https://example.com/event-producer"},
            {
                "zaak": self.zaak_url,
    ...
    def test_no_event_data(self):
        event = CloudEvent(
            {"type": ZAAK_GELINKT, "source": "https://example.com/event-producer"}, None
        )
    ...
    def test_runtime_error_in_handler(self):
        ...
        def raise_error(event):
            raise RuntimeError("Somehting really bad happened")
