...
class ZaakInformatieObjectSyncMixin:
    ...
    def setUp(self):
        ...
        patcher_sync_create = patch('openzaak.components.zaken.sync.signals.sync_create')
        self.mocked_sync_create = patcher_sync_create.start()
        self.addCleanup(patcher_sync_create.stop)

        patcher_sync_delete = patch('openzaak.components.zaken.sync.signals.sync_delete')
