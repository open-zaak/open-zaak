...
class BesluitScopeForbiddenCMISTests(AuthCheckMixin, APICMISTestCase):
    ...
    def test_cannot_read_without_correct_scope(self):
        ...
        bio = BesluitInformatieObjectFactory.create(besluit=besluit, informatieobject=eio_url)
        urls = [
            reverse("besluit-list"),
            reverse(besluit),
            reverse("besluitinformatieobject-list"),
...
class BioReadCMISTests(JWTAuthMixin, APICMISTestCase):
    ...
    def test_list_bio_limited_to_authorized_zaken(self):
        ...
        bio1 = BesluitInformatieObjectFactory.create(besluit=besluit1, informatieobject=eio1_url)
        # must not show up
        ...
        BesluitInformatieObjectFactory.create(besluit=besluit2, informatieobject=eio2_url)
    ...
    def test_create_bio_limited_to_authorized_besluiten(self):
        ...
        informatieobject_url = f"http://testserver{reverse(informatieobject)}"
        self.adapter.register_uri('GET', informatieobject_url, json=serialise_eio(informatieobject, informatieobject_url))
...
@tag("external-urls")
@override_settings(ALLOWED_HOSTS=["testserver"], CMIS_ENABLED=False)
